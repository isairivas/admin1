 ...
 * @author isai
 */
class EventoOrganizadores extends Controller {
    
    private $_name = 'evento-organizadores';
    private $_table ='evento_organizadores';
    private $_uploads;
    private $_primary = 'id';
    private $_title = 'Organizadores';
    
    function __construct() {
        parent::__construct();
        Param::setMenu('catalogos');
        Param::setSubMenu('eventos');
        Param::setTitle('Organizadores de Evento');
        $this->_uploads = PATH_UPLOADS.'images/eventos_organizadores/';
    ...
    public function index()
    {
       ...
       Param::navigate(array('Home' => HOME,'Eventos' => go('eventos'),$this->_title => go($this->_name,'view',array($eventoId)) ), 'icon-database');
       ...
       $aData = array(
        ...
        parent::getLoad()->loadView('eventos_organizadores/index',$aData);
    ...
    public function nuevo($eventoId = 'error') {
       ...
       $evento = $eventosDB->getById($eventoId);
        ...
        Param::setTitle('Nuevo organizador');
        Param::navigate(array('Home' => HOME,'Eventos' => go('eventos'),$this->_title => go($this->_name,'view',array($eventoId)),'Nuevo' => go($this->_name,'nuevo',array($eventoId)) ), 'icon-database');
        ...
        parent::getLoad()->loadView('containers/menu_sidebar');
        parent::getLoad()->loadView('containers/navigation');
        parent::getLoad()->loadView('eventos_organizadores/agregar',$aData);
        
        parent::getLoad()->loadView('containers/footer');
    }
    
    public function agregar(){
        $camposObligatorios = array('nombre','sys_status','eventos_id');
        $dataUpdate = array();
    ...
    public function edit($intId=0){
        ...
        Param::navigate(array('Home' => HOME,'Eventos' => go('eventos'),$this->_title => go($this->_name,'view',array($eventoId)),'Nuevo' => go($this->_name,'edit',array($intId)) ), 'icon-database');
        Param::setTitle('Editar '.$this->_title);
    ...
    public function save(){
        $camposObligatorios = array('nombre','sys_status','eventos_id',$this->_primary);
        $dataUpdate = array();
        foreach($_POST['registro'] as $field => $value ){
            if(!in_array($field, $camposObligatorios)){
                $dataUpdate[$field] = $value;
