...
class Banners extends Controller {
    ...
    function __construct() {
        ...
        Param::setSubMenu('banners');
        Param::setTitle('Banners');
        $this->_uploads = PATH_UPLOADS.'images/banners/';
    ...
    {
       Param::navigate(array('Home' => 'index.php','Banners' => go($this->_name)), 'icon-database');

       $modelDB = new Model($this->_table);
       $filtro = -1;
       $filtrosValidos = array('A','B','C','D');
       $records = array();
       if(isset($_GET['filtro']) && in_array($_GET['filtro'], $filtrosValidos)){
           $filtro = $_GET['filtro'];
           $records = $modelDB->toArray($modelDB->getByColumn('TIPO', $filtro));
       } else {
    ...
    public function nuevo(){
        ...
        Param::navigate(array('Home' => 'index.php','Banners' => go($this->_name),'Nuevo' => go($this->_name,'nuevo')), 'icon-database');
        Param::setTitle('Agregar nuevo Banner');
    ...
    public function agregar(){
        $camposObligatorios = array('nombre','sys_status','tipo');
        $dataUpdate = array();
        ...
        $dataUpdate['tipo'] = parent::getPost('registro.tipo', Filter::STRING, TRUE);
    ...
    public function edit($intId = 0){
        ...
        Param::setTitle('Editar Banner');
        
        Param::navigate(array('home' => HOME,'Banners' => go($this->_name),'Editar' => ''), 'icon-cogs');
        parent::getLoad()->setTemplate('templates/mooncake/');
        parent::getLoad()->loadView('containers/doctype');
        parent::getLoad()->loadView('containers/header');
        parent::getLoad()->loadView('containers/menu_sidebar');
