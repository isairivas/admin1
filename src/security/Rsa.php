...
private $primes;
    ...
    public function  __construct() {
        ...
        $this->initializatePrimes();
        $this->maxprimes = count($this->primes) - 1;
    }

    private function initializatePrimes() {
        $this->primes =
                array(4507, 4513, 4517, 4519, 4523, 4547, 4549, 4561, 4567, 4583, 4591, 4597,
...
*/
public function generateKeys ($show_debug=0){
    ...
    return serialize( base64_encode(serialize($keys)) );
}
...
public function encrypt ($m,$key=0) {
    if($key == 0){
        $key = $_SESSION['lw.rys.cid'];
    }
    $key = unserialize(base64_decode(unserialize($key)));
    $e = $key[1];
    $n = $key[0];
    $asci = array ();
    ...
    if ( !isset($resultmod) ){$resultmod = '';}
    if ( !isset($coded) ){$coded= '';}
...
M = X^D (mod N)
*/
public function decrypt ($c, $key = 0 ) {
    if($key == 0){
        $key = $_SESSION['lw.rys.cid'];
    }
    $key = unserialize(base64_decode(unserialize($key)));
